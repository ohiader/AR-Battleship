		...
		for (int i = 0; i < 10; i++)
		{
			for (int j = 0; j < 10; j++)
			{
				if (ToggleSettings.scaleToggle) {
					...
					playerBoard[i, j].transform.position += new Vector3 (-4, 1.0f, -6);
				} else {
					playerBoard[i,j] = (GameObject)Instantiate (gridPoint, new Vector3 ((float)i, 0, (float)j), Quaternion.identity);
					playerBoard[i, j].transform.position += new Vector3 (6.25f, 0.0f, 1.75f);
				}

                playerBoard[i,j].transform.parent = transform;

                playerBoard[i, j].transform.position = new Vector3(playerBoard[i, j].transform.position.x, 53, playerBoard[i, j].transform.position.z);
